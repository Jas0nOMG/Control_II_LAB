...
% Función de Transferencia RC: G(jw) = 1 / (1 + jwRC)
G_s = 1 ./ (1 + 1j * w_t * tau);
mag_teorica_db = 20 * log10(abs(G_s)); 
fase_teorica_deg = rad2deg(angle(G_s));

%% 5. Plot Inteligente: Teoría vs Experimental
% FILTRO VISUAL: Solo graficamos puntos donde la ganancia sea mayor al piso de ruido típico (-30dB aprox)
% Si la señal es muy débil, la fase es puro ruido.
umbral_ruido_db = -27; % Ajusta esto según tu gráfica (donde se aplana)
mask_valid = Ganancia_dB > umbral_ruido_db; 

figure('Color', 'w', 'Name', 'Bode: Teoria vs Experimental (Limpio)');

% --- Magnitud ---
subplot(2,1,1)
semilogx(f_teorica, mag_teorica_db, 'k--', 'LineWidth', 2); hold on;
% Graficamos TODO en magnitud para ver el piso de ruido
semilogx(f_Hz, Ganancia_dB, 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 6);
grid on; 
ylabel('Magnitud (dB)');
title('Magnitud ');
legend('Teoria', 'Experimental', 'Location', 'southwest');
xlim([min(f_teorica) max(f_teorica)]);

% --- Fase ---
subplot(2,1,2)
semilogx(f_teorica, fase_teorica_deg, 'k--', 'LineWidth', 2); hold on;
% AQUÍ ESTÁ EL TRUCO: Solo graficamos los puntos válidos (mask_valid)
semilogx(f_Hz(mask_valid), Fase_deg(mask_valid), 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 6);

% Opcional: Graficar los puntos malos en otro color (gris) para evidenciarlos
...
xlabel('Frecuencia (Hz)');
title('Fase ');
legend('Teoria', 'Datos Válidos', 'Ruido', 'Location', 'southwest');
xlim([min(f_teorica) max(f_teorica)]);
ylim([-100 10]);
